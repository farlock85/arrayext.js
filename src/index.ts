import { isObj, equal, OrderedArray, keyComparer, negate } from './helpers';
...
declare global {
    export interface Array<T> {
        ...
        removeAll(predicate: (value?: T, index?: number, list?: T[]) => boolean): Array<T>;
        ...
        skipWhile(predicate: (value?: T, index?: number, list?: T[]) => boolean): Array<T>;
        sum(): number;
        sum(transform: (value?: T, index?: number, list?: T[]) => number): number;
        sum(transform?: (value?: T, index?: number, list?: T[]) => number): number;
        take(amount: number): Array<T>;
        takeWhile(predicate: (value?: T, index?: number, list?: T[]) => boolean): Array<T>;
        union(list: Array<T>): Array<T>;
        where(predicate: (value?: T, index?: number, list?: T[]) => boolean): Array<T>;
        zip<U, T, TOut>(list: Array<U>, result: (first: T, second: U) => TOut): Array<TOut>;
    }
}
 ...
 */
Array.prototype.defaultIfEmpty = function<T>(defaultValue?: T): Array<T> {
 ...
 */
Array.prototype.distinctBy = function<T>(keySelector: (key: T) => string | number): Array<T> {
 ...
 */
Array.prototype.except = function<T>(source: Array<T>): Array<T> {
...
Array.prototype.groupJoin = function<T, U>(
    ...
    return this.select((x: T) => result(x, list.where(z => key1(x) === key2(z))));
};

/**
 * Produces the set intersection of two sequences by using the default equality comparer to compare values.
 */
Array.prototype.intersect = function<T>(source: Array<T>): Array<T> {
    return this.where((x: T) => source.contains(x));
};
 ...
 */
Array.prototype.ofType = function<U>(type: any): Array<U> {
    let typeName: string;
    switch (type) {
        case Number:
            typeName = 'number';
    ...
    return typeName === null
        ? (this.where((x: any) => x instanceof type) as Array<U>).cast<U>()
        : (this.where((x: any) => typeof x === typeName) as Array<U>).cast<U>();
};
 ...
 */
Array.prototype.selectMany = function<T, TOut extends Array<any>>(
    selector: (element: T, index: number) => TOut
): TOut {
 ...
 */
Array.prototype.sequenceEqual = function<T>(list: Array<T>): boolean {
...
Array.prototype.skipWhile = function<T>(
    ...
    return this.skip(
        this.aggregate((ac: number) => (predicate(this.elementAt(ac)) ? ++ac : ac), 0)
    );
};
 ...
 */
Array.prototype.union = function<T>(list: Array<T>): Array<T> {
